...
is about building, packaging, and testing the app, not about using it.
...
Code signing is not wired up. The published Windows installers at `v0.1.0-windows` carry
no Authenticode certificate, so SmartScreen warns on first run. Local packages are unsigned
for the same reason.
